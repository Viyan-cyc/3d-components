...
/** 角度归一化的 epsilon（接近零时视为 0）。 */
const ANGLE_EPSILON = 1e-6;

/** 旋转弧半径（fixed 模式）。 */
const ARC_RADIUS_FIXED = 0.65;

/** 旋转弧半径系数（非 fixed 模式下 scale 的乘数）。 */
const ARC_RADIUS_FACTOR = 0.65;

/** 管径宽度系数（fixed 模式下 lineWidth / scale 的乘数）。 */
const TUBE_WIDTH_FACTOR = 1.6;

/** 管径除数（非 fixed 模式下 scale 的除数）。 */
const TUBE_DIVISOR = 14;

/** 命中环管径最小比例（scale 的乘数）。 */
const HIT_TUBE_MIN_FACTOR = 0.1;
...
const minimizeAngle = (angle: number): number => {
  ...
  if (Math.abs(result) < ANGLE_EPSILON) {
    return 0.0;
  }
  if (result < 0.0) {
    result += 2 * Math.PI;
...
export class AxisRotator extends PivotHandle {
  ...
  protected _build(): void {
    ...
    const r = fixed ? ARC_RADIUS_FIXED : scale * ARC_RADIUS_FACTOR;
    const tube = fixed ? (lineWidth / scale) * TUBE_WIDTH_FACTOR : scale / TUBE_DIVISOR;

    this.createAnnotation([r, r, 0]);
    this.buildHitTorus({
      r, tube, opacity, depthTest, renderOrder, scale,
    });
    this.buildVisibleArc(r, lineWidth, opacity, depthTest, renderOrder);
  }

  /** 构建不可见命中环（管径放大以便拾取）。 */
  private buildHitTorus(params: {
    r: number; tube: number; opacity: number;
    depthTest: boolean; renderOrder: number; scale: number;
  }): void {
    const {
      r, tube, opacity, depthTest, renderOrder, scale,
    } = params;
    const hitTube = Math.max(tube, scale * HIT_TUBE_MIN_FACTOR);
    const hitGeo = new THREE.TorusGeometry(r, hitTube, TORUS_RADIAL_SEGMENTS, TORUS_TUBULAR_SEGMENTS, Math.PI / 2);
    const hitMesh = new THREE.Mesh(
      hitGeo,
      new THREE.MeshBasicMaterial({
        transparent: true, opacity, depthTest, visible: false, side: THREE.DoubleSide,
      }),
    );
  ...
  /** 构建可见四分之一圆弧。 */
  private buildVisibleArc(
    r: number, lineWidth: number,
  ...
  onPointerDown(sample: PointerSample): void {
    ...
    this.clickInfo = {
      clickPoint, origin, e1, e2, normal, plane,
    };
    this.setAnnotationText(`${toDegrees(this.angle).toFixed(0)}º`);
    this.config.onDragStart({
      component: 'Rotator', axis: this.axis, origin, directions: [e1, e2, normal],
    });
  }
  ...
  onPointerMove(sample: PointerSample): void {
    ...
    if (sample.shiftKey) {
      degrees = Math.round(degrees / SNAP_DEGREES) * SNAP_DEGREES;
