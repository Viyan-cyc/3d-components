...
/** 注释徽标屏幕投影用的临时向量。 */
const projV = /* @__PURE__ */ new THREE.Vector3();
...
export abstract class PivotHandle extends THREE.Group {
  ...
  update(_delta: number, camera: THREE.Camera, renderer: THREE.WebGLRenderer): void {
    ...
    projV.copy(this.annotationAnchor);
    this.objGroup.localToWorld(projV);
    ...
    const x = rect.left + (projV.x * NDC_TO_SCREEN + NDC_TO_SCREEN) * rect.width;
    const y = rect.top + (-projV.y * NDC_TO_SCREEN + NDC_TO_SCREEN) * rect.height;
    div.style.left = `${x}px`;
    div.style.top = `${y}px`;
  ...
  protected createAnnotation(anchorLocal: [number, number, number]): void {
    ...
    if (this.config.annotationsClass) {
      div.className = this.config.annotationsClass;
    }
  ...
  protected setAnnotationText(text: string | null): void {
    ...
    if (text === null) {
      ...
      if (this.annotationDiv.parentNode) {
        this.annotationDiv.style.display = 'block';
      }
    }
  }
  ...
  dispose(): void {
    ...
    if (this.annotationDiv?.parentNode) {
      this.annotationDiv.parentNode.removeChild(this.annotationDiv);
    }
