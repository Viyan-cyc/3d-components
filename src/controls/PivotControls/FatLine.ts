...
/** 多边形偏移因子（避免与操控件 mesh 互相 z-fighting）。 */
const POLYGON_OFFSET_FACTOR = -10;

/** Vector3 展平为扁平数组时每个点占用的分量数（x, y, z）。 */
...
export class FatLine extends Line2 {
  ...
  constructor(options: FatLineOptions) {
    ...
    const material = new LineMaterial({
      ...
      resolution: new THREE.Vector2(DEFAULT_RESOLUTION, DEFAULT_RESOLUTION),
    });
    if (options.polygonOffset) {
      material.polygonOffset = true;
      material.polygonOffsetFactor = options.polygonOffsetFactor ?? POLYGON_OFFSET_FACTOR;
  ...
  private static toFlat(points: THREE.Vector3[]): number[] {
    const flat = new Array(points.length * COMPONENTS_PER_POINT);
    for (let i = 0; i < points.length; i++) {
      const p = points[i];
      flat[i * COMPONENTS_PER_POINT] = p.x;
      flat[i * COMPONENTS_PER_POINT + 1] = p.y;
      flat[i * COMPONENTS_PER_POINT + 2] = p.z;
    }
    return flat;
  }
