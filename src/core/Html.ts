...
const DEG_TO_RAD_FACTOR = Math.PI / DEGREES_IN_HALF_CIRCLE;
const EPSILON_THRESHOLD = 1e-10;
const MATRIX_SIZE = 16;
const LAST_MATRIX_INDEX = 15;
const DEFAULT_Z_INDEX_MAX = 16777271;
const DEFAULT_EPS = 0.001;
const DISTANCE_FACTOR_DIVISOR = 400;
const DEFAULT_DISTANCE_FACTOR_NUM = 10;

// ────────────────────────────── Shared Vectors ──────────────────────────────

const tmpV1 = /* @__PURE__ */ new THREE.Vector3();
const tmpV2 = /* @__PURE__ */ new THREE.Vector3();
const tmpV3 = /* @__PURE__ */ new THREE.Vector3();
const tmpV4 = /* @__PURE__ */ new THREE.Vector2();
...
const defaultCalculatePosition = (
  ...
  const objectPos = tmpV1.setFromMatrixPosition(object.matrixWorld);
...
const isObjectBehindCamera = (object: THREE.Object3D, camera: THREE.Camera): boolean => {
  const objectPos = tmpV1.setFromMatrixPosition(object.matrixWorld);
  const cameraPos = tmpV2.setFromMatrixPosition(camera.matrixWorld);
  const deltaCamObj = objectPos.sub(cameraPos);
  const camDir = camera.getWorldDirection(tmpV3);
...
const isObjectVisible = (
  ...
  const elPos = tmpV1.setFromMatrixPosition(object.matrixWorld);
  const screenPos = elPos.clone();
  screenPos.project(camera);
  tmpV4.set(screenPos.x, screenPos.y);
  raycaster.setFromCamera(tmpV4, camera);
...
const objectScale = (object: THREE.Object3D, camera: THREE.Camera): number => {
  if (camera instanceof THREE.OrthographicCamera) {
    return camera.zoom;
  } else if (camera instanceof THREE.PerspectiveCamera) {
    const objectPos = tmpV1.setFromMatrixPosition(object.matrixWorld);
    const cameraPos = tmpV2.setFromMatrixPosition(camera.matrixWorld);
    const vFOV = camera.fov * DEG_TO_RAD_FACTOR;
...
const epsilon = (value: number): number => {
  if (Math.abs(value) < EPSILON_THRESHOLD) {
    return 0;
  }
  return value;
};
...
const getCSSMatrix = (
  ...
  for (let i = 0; i < MATRIX_SIZE; i++) {
    const separator = i === LAST_MATRIX_INDEX ? ')' : ',';
    matrix3d += epsilon(multipliers[i] * matrix.elements[i]) + separator;
  }
...
const getCameraCSSMatrix = (matrix: THREE.Matrix4): string =>
  getCSSMatrix(matrix, CAMERA_MULTIPLIERS);

const getObjectCSSMatrix = (matrix: THREE.Matrix4, factor: number): string => {
  const f = factor;
  const multipliers = [
    1 / f, 1 / f, 1 / f, 1,
    -1 / f, -1 / f, -1 / f, -1,
...
export class Html extends THREE.Group implements IUpdatable, IDisposable {
  ...
  private _calculatePosition!: CalculatePositionFn;
  ...
  constructor(options: HtmlOptions = {}) {
    ...
    this._el = options.el ?? document.createElement(options.as ?? 'div');
    this._contentRef = Html.extractContent(this._el);
    this._initFromOptions(options);
  }

  /** Apply base ComponentOptions (name, visible, userData). */
  private _applyBaseOptions(options: HtmlOptions): void {
    if (options.name) {
  ...
  /** Initialize all fields and occlusion from options. */
  private _initFromOptions(options: HtmlOptions): void {
    this._portal = options.portal ?? null;
    this._prepend = options.prepend ?? false;
    this._center = options.center ?? false;
    this._fullscreen = options.fullscreen ?? false;
    this._eps = options.eps ?? DEFAULT_EPS;
    this._distanceFactor = options.distanceFactor;
    this._sprite = options.sprite ?? false;
    this._transform = options.transform ?? false;
    this._zIndexRange = options.zIndexRange ?? [DEFAULT_Z_INDEX_MAX, 0] as [number, number];
    this._calculatePosition = options.calculatePosition ?? defaultCalculatePosition;
    this._wrapperClass = options.wrapperClass;
    this._pointerEvents = options.pointerEvents ?? 'auto';
    this._style = options.style;
    this._className = options.className;
    this._occlude = options.occlude ?? false;
    this._onOcclude = options.onOcclude;
    this._isRayCastOcclusion = this._computeIsRayCastOcclusion(this._occlude);
    this._initOcclusionMesh({
      occlude: this._occlude,
      occludeGeometry: options.occludeGeometry,
      occludeMaterial: options.occludeMaterial,
  ...
  /** Extract existing child nodes into a new content div. */
  private static extractContent(el: HTMLElement): HTMLDivElement {
    const contentRef = document.createElement('div');
    while (el.firstChild) {
      contentRef.appendChild(el.firstChild);
    }
    return contentRef;
  }

  /** Initialize the occlusion mesh if needed. */
  private _initOcclusionMesh(opts: {
    occlude: OccludeMode;
    occludeGeometry?: THREE.BufferGeometry;
    occludeMaterial?: THREE.Material;
    transform: boolean;
    ...
    if (opts.occlude && !this._isRayCastOcclusion && opts.occlude !== 'blending') {
      const geometry = opts.occludeGeometry ?? new THREE.PlaneGeometry();
      const material =
        opts.occludeMaterial ??
        new THREE.ShaderMaterial({
          side: THREE.DoubleSide,
          vertexShader: opts.transform ? undefined : OCCLUDE_VERTEX_SHADER,
          fragmentShader: OCCLUDE_FRAGMENT_SHADER,
        });
      this._occlusionMesh = new THREE.Mesh(geometry, material);
      this._occlusionMesh.castShadow = opts.castShadow;
      this._occlusionMesh.receiveShadow = opts.receiveShadow;
  ...
  get transformOuter(): HTMLDivElement | null {
    return this._transformOuterRef;
  }

  /** The transform inner wrapper div (only in transform mode). */
  get transformInner(): HTMLDivElement | null {
    return this._transformInnerRef;
  }

  /** The occlusion mesh (if created). */
  get occlusionMesh(): THREE.Mesh | null {
    return this._occlusionMesh;
  }
  ...
  mount(renderer: THREE.WebGLRenderer): void {
    ...
    if (this._wrapperClass) {
      this._el.className = this._wrapperClass;
    }
    this._buildContent();
  ...
  update(_delta: number, camera?: THREE.Camera, renderer?: THREE.WebGLRenderer): void {
    ...
    const size = renderer.getSize(tmpV4);
    const width = size.x;
    const height = size.y;
    ...
    const cameraZoom = this._resolveCameraZoom(camera);
    ...
    if (
      ...
      this._updateVisibility(camera, vec, cameraZoom);
    }

    this._updateOcclusionMeshSize(camera);
  }

  /** Resolve zoom from camera type without nested ternary. */
  private _resolveCameraZoom(camera: THREE.Camera): number {
    if (camera instanceof THREE.OrthographicCamera) {
      return camera.zoom;
    }
  ...
  /** Update visibility, z-index, and position when dirty. */
  private _updateVisibility(
    ...
    if (this._transform) {
      this._updateTransformMode(camera, vec[0], vec[1]);
    } else {
  ...
  /** Resolve z-index range without nested ternary. */
  private _resolveZRange(): [number, number] {
    const halfRange = Math.floor(this._zIndexRange[0] / 2);
    if (!this._occlude) {
      return this._zIndexRange;
    }
    if (this._isRayCastOcclusion) {
      return [this._zIndexRange[0], halfRange];
    }
    return [halfRange - 1, 0];
  }
  ...
  dispose(): void {
    ...
    this._disposeMesh(this._occlusionMesh);
  ...
  private _buildContent(): void {
    ...
    this._contentRef.style.position = 'relative';

    if (this._className) {
      this._contentRef.className = this._className;
    }
    // Only apply _style if it hasn't been manually set by user via setStyle()
    // (setStyle writes directly, so we respect that)

    if (this._transform) {
      this._buildTransformContent();
    } else {
      this._buildNonTransformContent();
    }
  }

  /** Build DOM for transform mode: _el -> outerRef -> innerRef -> _contentRef. */
  private _buildTransformContent(): void {
  ...
  /** Build DOM for non-transform mode: _el -> _contentRef. */
  private _buildNonTransformContent(): void {
  ...
  private _updateTransformMode(
    ...
    const cameraTransform = this._buildCameraTransform(camera, fov, isOrthographic);
    const matrix = this._computeSpriteMatrix(camera);
    ...
    if (this._transformOuterRef && this._transformInnerRef) {
      ...
      const factor = 1 / ((this._distanceFactor ?? DEFAULT_DISTANCE_FACTOR_NUM) / DISTANCE_FACTOR_DIVISOR);
      this._transformInnerRef.style.transform = getObjectCSSMatrix(matrix, factor);
    }
  }

  /** Build the camera CSS transform string. */
  private _buildCameraTransform(
    camera: THREE.Camera,
    fov: number,
    ...
    if (isOrthographic) {
      ...
      const tx = epsilon(-(ortho.right + ortho.left) / 2);
      const ty = epsilon((ortho.top + ortho.bottom) / 2);
      return `scale(${fov})translate(${tx}px,${ty}px)`;
    }
    return `translateZ(${fov}px)`;
  }

  /** Compute the world matrix, adjusted for sprite mode if needed. */
  private _computeSpriteMatrix(camera: THREE.Camera): THREE.Matrix4 {
  ...
  private _updateOcclusionMeshSize(camera: THREE.Camera): void {
    if (!this._occlusionMesh || this._isRayCastOcclusion || this._isMeshSizeSet) {
      return;
    }

    // Always use _contentRef for dimensions — it holds the actual user content
    if (this._transform) {
      this._updateTransformOcclusionSize(camera);
    } else {
      this._updateNonTransformOcclusionSize(camera);
    }
  }

  /** Update occlusion mesh size in transform mode. */
  private _updateTransformOcclusionSize(camera: THREE.Camera): void {
  ...
  /** Update occlusion mesh size in non-transform mode. */
  private _updateNonTransformOcclusionSize(camera: THREE.Camera): void {
    if (!this._contentRef.clientWidth || !this._contentRef.clientHeight) {
      return;
    }

    const factor = this._computeOcclusionFactor(camera);
  ...
  private _computeOcclusionFactor(camera: THREE.Camera): number {
    if (camera instanceof THREE.PerspectiveCamera) {
      const cameraPos = tmpV1.setFromMatrixPosition(camera.matrixWorld);
      const objectPos = tmpV2.setFromMatrixPosition(this.matrixWorld);
      const dist = objectPos.distanceTo(cameraPos);
      const vFOV = camera.fov * DEG_TO_RAD_FACTOR;
