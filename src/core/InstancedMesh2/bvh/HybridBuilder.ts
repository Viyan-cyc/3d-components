...
import { areaBox, areaFromTwoBoxes, expandBoxByMargin, getLongestAxis, isBoxInsideBox, isExpanded, unionBox, unionBoxChanged } from './boxUtils';
...
export class HybridBuilder<N = {}, L = {}> implements IBVHBuilder<N, L> {
    ...
    public createFromArray(objects: L[], boxes: FloatArray[], onLeafCreation?: onLeafCreationCallback<N, L>, margin = 0): void {
        ...
        function buildNode(offset: number, count: number, parent: BVHNode<N, L> | null): BVHNode<N, L> {
            ...
            const node = { box, parent, left: null, right: null } as BVHNode<N, L>;
        ...
        function computeBoxCentroid(offset: number, count: number): FloatArray {
            ...
            centroid[0] = Infinity; centroid[1] = -Infinity;
            centroid[2] = Infinity; centroid[3] = -Infinity;
            centroid[4] = Infinity; centroid[5] = -Infinity;

            for (let i = offset; i < end; i++) {
                const boxToCheck = boxes[i];
                const xMin = boxToCheck[0], xMax = boxToCheck[1];
                const yMin = boxToCheck[2], yMax = boxToCheck[3];
                const zMin = boxToCheck[4], zMax = boxToCheck[5];
        ...
        function updateSplitData(): void {
            axis = getLongestAxis(centroid) * 2;
    ...
    protected createLeafNode(object: L, box: FloatArray): BVHNode<N, L> {
        const node = { box, object, parent: null, left: null, right: null } as BVHNode<N, L>;
        return node;
    }

    protected createInternalNode(parent: BVHNode<N, L> | null, sibling: BVHNode<N, L>, leaf: BVHNode<N, L>): BVHNode<N, L> {
        const node = { parent, left: sibling, right: leaf, box: new this._typeArray(6) } as BVHNode<N, L>;
        return node;
    }
    ...
    protected findBestSibling(leafBox: FloatArray): BVHNode<N, L> | null {
        ...
        do {
            ...
            if (currentCostL > currentCostR) {
                if (bestCost > currentCostR) { bestNode = nodeR; bestCost = currentCostR; }
            } else if (bestCost > currentCostL) {
                bestNode = nodeL; bestCost = currentCostL;
            }
    ...
    protected refitAndRotate(node: BVHNode<N, L>, sibling: BVHNode<N, L>): void {
        ...
        while ((node = node.parent!)) {
            ...
            if (!isExpanded(originalNodeBox, nodeBox)) return;
            ...
            if (right.object === undefined) {
                ...
                if (diffRR > diffRL) {
                    if (diffRR > 0) { nodeSwap1 = left; nodeSwap2 = RR; bestCost = diffRR; }
                } else if (diffRL > 0) {
            ...
            if (left.object === undefined) {
                ...
                if (diffLR > diffLL) {
                    if (diffLR > bestCost) { nodeSwap1 = right; nodeSwap2 = LR; }
                } else if (diffLL > bestCost) {
                    nodeSwap1 = right; nodeSwap2 = LL;
                }
            }

            if (nodeSwap1 !== null && nodeSwap2 !== null) this.swap(nodeSwap1, nodeSwap2);
        }
    }
