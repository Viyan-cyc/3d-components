...
export class Frustum {
    ...
    public setFromProjectionMatrix(mat: FloatArray | number[]): this {
        this.updatePlane(0, mat[3] + mat[0], mat[7] + mat[4], mat[11] + mat[8], mat[15] + mat[12]); // Left
        this.updatePlane(1, mat[3] - mat[0], mat[7] - mat[4], mat[11] - mat[8], mat[15] - mat[12]); // Right
        this.updatePlane(2, mat[3] - mat[1], mat[7] - mat[5], mat[11] - mat[9], mat[15] - mat[13]); // Top
        this.updatePlane(3, mat[3] + mat[1], mat[7] + mat[5], mat[11] + mat[9], mat[15] + mat[13]); // Bottom
        this.updatePlane(4, mat[3] - mat[2], mat[7] - mat[6], mat[11] - mat[10], mat[15] - mat[14]); // Far

        if (this.coordinateSystem === WebGLCoordinateSystem) {
            this.updatePlane(5, mat[3] + mat[2], mat[7] + mat[6], mat[11] + mat[10], mat[15] + mat[14]); // Near
        } else if (this.coordinateSystem === WebGPUCoordinateSystem) {
            this.updatePlane(5, mat[2], mat[6], mat[10], mat[14]); // Near
        }
    ...
    public intersectsBoxMask(box: FloatArray, mask: number): number {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            if (minDot < -planeConstant) return -1;

            const maxDot = (px * xMax) + (py * yMax) + (pz * zMax);
            if (maxDot > -planeConstant) mask ^= bit;
        }
        return mask;
    }
    ...
    /** Frustum test accounting for BVH margin - use in `onFrustumIntersectionCallback` if margin > 0. */
