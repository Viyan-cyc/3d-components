...
export const vec3ToArray = function (vector: Vector3Like, target: FloatArray): FloatArray {
  target[0] = vector.x;
  target[1] = vector.y;
  target[2] = vector.z;
  return target;
...
export const box3ToArray = function (box: Box3Like, target: FloatArray): FloatArray {
  const min = box.min;
  const max = box.max;
  target[0] = min.x;
  target[1] = max.x;
