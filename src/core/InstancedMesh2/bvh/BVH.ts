...
import type { CoordinateSystem } from './frustum';
import { Frustum, WebGLCoordinateSystem } from './frustum';
...
export class BVH<N, L> {
    ...
    public intersectsRay(dir: FloatArray, origin: FloatArray, onIntersection: onIntersectionCallback<L>, near = 0, far = Infinity): boolean {
        ...
        dirInv[0] = 1 / dir[0]; dirInv[1] = 1 / dir[1]; dirInv[2] = 1 / dir[2];
        sign[0] = dirInv[0] < 0 ? 1 : 0; sign[1] = dirInv[1] < 0 ? 1 : 0; sign[2] = dirInv[2] < 0 ? 1 : 0;
    ...
    public frustumCullingLOD(projectionMatrix: FloatArray | number[], cameraPosition: FloatArray, levels: FloatArray, onIntersection: onFrustumIntersectionLODCallback<N, L>): void {
        ...
        _frustumCullingLOD(this.root, 0b111111, null as any);

        function _frustumCullingLOD(node: BVHNode<N, L>, mask: number, level: number | null): void {
            ...
            if (node.object !== undefined) {
                if (frustum.isIntersected(nodeBox, mask)) {
                    onIntersection(node, level as number, frustum, mask);
                }
                return;
            }
        ...
        function showAll(node: BVHNode<N, L>, level: number | null): void {
            if (level === null) {
                level = getLevel(node.box);
            }

            if (node.object !== undefined) {
                onIntersection(node, level as number, frustum, 0);
                return;
            }
            showAll(node.left!, level);
            showAll(node.right!, level);
        ...
        function getLevel(nodeBox: FloatArray): number {
            ...
            for (let i = levels.length - 1; i > 0; i--) {
                if (max >= levels[i]) {
                    return min >= levels[i] ? i : null as any;
                }
            }
    ...
    public closestPointToPoint(point: FloatArray, onClosestDistance?: onClosestDistanceCallback<L>): number {
        if (this.root === null) return undefined as any;
        ...
        function _closestPointToPoint(node: BVHNode<N, L>): void {
            if (node.object !== undefined) {
                if (onClosestDistance) {
                    ...
                    bestDistance = minDistanceSqPointToBox(node.box, point);
                }
                return;
            }
