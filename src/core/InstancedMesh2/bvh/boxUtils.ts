...
export function unionBoxChanged(A: FloatArray, B: FloatArray, target: FloatArray): boolean {
    ...
    if (target[0] > t0) { target[0] = t0; changed = true; }
    if (target[1] < t1) { target[1] = t1; changed = true; }
    if (target[2] > t2) { target[2] = t2; changed = true; }
    if (target[3] < t3) { target[3] = t3; changed = true; }
    if (target[4] > t4) { target[4] = t4; changed = true; }
    if (target[5] < t5) { target[5] = t5; changed = true; }
...
export function isExpanded(A: FloatArray, target: FloatArray): boolean {
    ...
    if (target[0] > A[0]) { target[0] = A[0]; expanded = true; }
    if (target[1] < A[1]) { target[1] = A[1]; expanded = true; }
    if (target[2] > A[2]) { target[2] = A[2]; expanded = true; }
    if (target[3] < A[3]) { target[3] = A[3]; expanded = true; }
    if (target[4] > A[4]) { target[4] = A[4]; expanded = true; }
    if (target[5] < A[5]) { target[5] = A[5]; expanded = true; }
...
export function minMaxDistanceSqPointToBox(box: FloatArray, point: FloatArray): { min: number; max: number } {
    let dXmin: number, dXmax: number, dYmin: number, dYmax: number, dZmin: number, dZmax: number;

    const xMin = box[0] - point[0];
    const xMax = point[0] - box[1];

    if (xMin > xMax) { dXmin = xMin; dXmax = xMax; }
    ...
    if (yMin > yMax) { dYmin = yMin; dYmax = yMax; }
    ...
    if (zMin > zMax) { dZmin = zMin; dZmax = zMax; }
