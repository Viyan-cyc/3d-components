...
/** Dot-product threshold above which two segments are considered collinear. */
const COLLINEAR_THRESHOLD = 0.999999;

/** Half — used for max-tangent calculation. */
const HALF = 0.5;

/** Dimension count for cross-product in 2D (z-component). */
const CROSS_DIMENSION = 3;

// ===================== dedupe =====================

/** Remove consecutive duplicate points (within EPS). */
export const dedupe = (input: V2[]): V2[] => {
  const out: V2[] = [];
  for (const p of input) {
    const last = out[out.length - 1];
    if (last) {
      if (last.distanceTo(p) > EPS) {
        out.push(p);
      }
    } else {
      out.push(p);
...
export const radiusAt = (radius: number | number[], i: number): number =>
  Array.isArray(radius) ? radius[i] ?? 0 : radius;
 ...
 */
export const radiusAtWithDefault = (
...
export const computeCorner = (prev: V2, cur: V2, next: V2, r: number): Corner | null => {
  ...
  const maxT = Math.min(len1, len2) * HALF;
  if (t > maxT) {
    t = maxT;
  }
  if (t <= EPS) {
  ...
  return {
    ...
    delta: turnSign * alpha,
  };
};

/** Round the corners of a polyline into a dense point list. */
export const filletPolyline = (input: V2[], radius: number | number[], segments: number, close: boolean): V2[] => {
  ...
  if (close && n >= CROSS_DIMENSION) {
    for (let i = 0; i < n; i++) {
      handle(i, pts[(i - 1 + n) % n], pts[i], pts[(i + 1) % n]);
    }
  } else {
    out.push(pts[0].clone());
    for (let i = 1; i < n - 1; i++) {
      handle(i, pts[i - 1], pts[i], pts[i + 1]);
    }
...
export const signedArea = (poly: V2[]): number => {
  let a = 0;
  const n = poly.length;
  for (let i = 0; i < n; i++) {
    const p = poly[i];
...
export const ensureCCW = (poly: V2[]): void => {
  if (signedArea(poly) < 0) {
    poly.reverse();
  }
};

export const ensureCW = (poly: V2[]): void => {
  if (signedArea(poly) > 0) {
    poly.reverse();
  }
};

export const traceContour = (path: THREE.Path | THREE.Shape, poly: V2[]): void => {
  if (poly.length === 0) {
    return;
  }
...
export const toShape2D = (p: Vec3Tuple): V2 =>
  new THREE.Vector2(p[0], -p[2]);
