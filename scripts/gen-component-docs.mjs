...
function parseRows4(tbl) {
  return parseRows(tbl, 4).map((c) => ({
    name: c[0],
    type: c[1],
    default: c[2],
    description: c[3],
...
function parseRows3(tbl) {
  return parseRows(tbl, 3).map((c) => ({ name: c[0], type: c[1], description: c[2] }));
}
...
function parseComponent(name) {
  ...
  const extends_ = stripTags(
    firstMatch(head, /<span class="tag">extends\s*<code>([\s\S]*?)<\/code><\/span>/),
  );
  // 构造签名 = head 里第一个 div.sig（无嵌套 div，非贪婪到首个 </div> 安全）
  const constructor = stripTags(
    firstMatch(head, /<div class="sig">([\s\S]*?)<\/div>/),
  );
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    if (sig) methods.push({ signature: sig, description: desc });
  }
...
function main() {
  ...
  for (const name of COMPONENTS) {
    try {
      ...
      out.push(doc);
      console.log(
        `[gen] ✓ ${name}: ${doc.options.length} options, ${doc.dataTypes.length} dataTypes, ` +
